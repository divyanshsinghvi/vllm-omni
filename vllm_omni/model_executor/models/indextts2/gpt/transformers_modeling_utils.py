...
from vllm_omni.model_executor.models.indextts2.gpt.transformers_generation_utils import GenerationMixin
...
def check_support_param_buffer_assignment(model_to_load, state_dict, start_prefix=""):
    ...
    # For cases when the `state_dict` doesn't contain real weights to the model
...
def _load_state_dict_into_model(model_to_load, state_dict, start_prefix, assign_to_params_buffers=False):
    ...
    if renamed_keys:
        warning_msg += (
            "contains parameters that have been renamed internally"
            " (a few are listed below but more are present"
            " in the model):\n"
        )
        for old_key, new_key in renamed_keys.items():
            warning_msg += f"* `{old_key}` -> `{new_key}`\n"
        warning_msg += (
            "If you are using a model from the Hub, consider"
            " submitting a PR to adjust these weights and"
            " help future users."
        )
...
def _load_state_dict_into_meta_model(
    ...
    for key in state_dict.keys():
        ...
        # To reproduce `_load_state_dict_into_model` behaviour, we need to manually rename parametrized weight norm, if
        # necessary.
    ...
    for param_name, param in state_dict.items():
        ...
        # We convert floating dtypes to the `dtype` passed except for float8_e4m3fn type. We also want to keep the
        # buffers/params
        ...
        # For compatibility with PyTorch load_state_dict which converts state dict dtype to existing dtype in model, and
        # which
        # uses `param.copy_(input_param)` that preserves the contiguity of the parameter in the model.
        # Reference: https://github.com/pytorch/pytorch/blob/db79ceb110f6646523019a59bbd7b838f43d4a86/torch/nn/modules/m
        # odule.py#L2040C29-L2040C29
    ...
    def num_parameters(self, only_trainable: bool = False, exclude_embeddings: bool = False) -> int:
        ...
        if is_loaded_in_4bit:
            if is_bitsandbytes_available():
                ...
                raise ValueError(
                    "bitsandbytes is not installed but it seems that the model has been loaded in 4bit precision,"
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def _autoset_attn_implementation(
        ...
        Automatically checks and dispatches to a default attention implementation. In order of priority:
            1. An implementation specified in
            `config._attn_implementation` (due for example to the
            argument attn_implementation="sdpa" in from_pretrained).
            2. DEPRECATED: if use_flash_attention_2 is set to `True`
            and `flash_attn` is available, flash attention.
        ...
        if hasattr(config, "_attn_implementation_internal") and config._attn_implementation_internal is not None:
            if config._attn_implementation != "flash_attention_2" and use_flash_attention_2:
                raise ValueError(
                    f"Both attn_implementation="
                    f'"{config._attn_implementation}" and'
                    f" `use_flash_attention_2=True` were used"
                    f" when loading the model, which are not"
                    f" compatible."
            ...
            if not isinstance(config._attn_implementation, dict) and config._attn_implementation not in [
                ...
                message = (
                    f"Specified `attn_implementation="
                    f'"{config._attn_implementation}"` is not'
                    f" supported. The only possible arguments"
                    f' are `attn_implementation="eager"`'
                ...
                if cls._supports_sdpa:
                    message += (
                        ...
                        " (implementation using torch.nn.functional"
                        ".scaled_dot_product_attention)"
                    )
                raise ValueError(message + ".")

            # If a config is passed with a preset attn_implementation, we skip the automatic dispatch and use the user-
            # provided config, with hard checks that the requested attention implementation is available.
        ...
        # dict.
        # Later each sub-module will dispatch with its own attn impl, by calling
        # `XXXModel._from_config(config.text_config)`
        # If any of sub-modules doesn't support requested attn, an error will be raised. See
        # https://github.com/huggingface/transformers/pull/32238
        for key in config:
    ...
    def _check_and_enable_flash_attn_2(
        ...
        If all checks pass and `hard_check_only` is False, the method
        will set the config attribute `attn_implementation` to
        "flash_attention_2" so that the model can initialize the
        correct attention module.
        """
        if not cls._supports_flash_attn_2:
            raise ValueError(
                f"{cls.__name__} does not support Flash Attention 2.0 yet. Please request to add support where"
                f" the model is hosted, on its model hub page:"
                f"https://huggingface.co/{config._name_or_path}/discussions/new"
        ...
        if not is_flash_attn_2_available():
            ...
            if torch.version.cuda:
                if flash_attention_version < version.parse("2.1.0"):
                    raise ImportError(
                        f"{preface} you need flash_attn package version"
                        f" to be greater or equal than 2.1.0."
                        f" Detected version"
                        f" {flash_attention_version}."
                        f" {install_message}"
                    )
                elif not torch.cuda.is_available():
                    raise ValueError(
                        f"{preface} Flash Attention 2 is not available on CPU. Please make sure torch can access a CUDA"
                        f"device."
                        f"{preface} Flash Attention 2 is not available on CPU. Please make sure torch can access a CUDA"
                        f"device."
                    )
                ...
                if flash_attention_version < version.parse("2.0.4"):
                    raise ImportError(
                        f"{preface} you need flash_attn package version to be greater or equal than 2.0.4. Make sure to"
                        f"have that version installed - detected version {flash_attention_version}. {install_message}"
                        f"{preface} you need flash_attn package version to be greater or equal than 2.0.4. Make sure to"
                        f"have that version installed - detected version {flash_attention_version}. {install_message}"
                    )
                else:
        ...
        if _is_bettertransformer:
            raise ValueError(
                "Flash Attention 2 and BetterTransformer API are not compatible. Please make sure to disable"
                "BetterTransformers by doing model.reverse_bettertransformer()"
            )

        if torch_dtype is None:
            logger.warning_once(
                "You are attempting to use Flash Attention 2.0 without specifying a torch dtype. This might lead to"
                "unexpected behaviour"
            )
        elif torch_dtype is not None and torch_dtype not in [torch.float16, torch.bfloat16]:
            logger.warning_once(
                "Flash Attention 2.0 only supports torch.float16 and torch.bfloat16 dtypes, but"
                f" the current dype in {cls.__name__} is"
                f" {torch_dtype}. You should run training or"
                f" inference using Automatic Mixed-Precision"
                f" via the `with torch.autocast("
                f"device_type='torch_device'):` decorator,"
                " or load the model with the `torch_dtype` argument. Example: `model ="
                'AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="flash_attention_2",'
        ...
        # The check `torch.empty(0).device.type != "cuda"` is needed as the model may be initialized after
        # `torch.set_default_device` has been called,
        # or the model may be initialized under the context manager `with torch.device("cuda"):`.
        if check_device_map and device_map is None and torch.empty(0).device.type != "cuda":
            if torch.cuda.is_available():
                logger.warning_once(
                    "You are attempting to use Flash Attention 2.0 with a model not initialized on GPU. Make sure to"
                    "move the model to GPU"
                    " after initializing it on CPU with `model.to('cuda')`."
                )
            else:
                raise ValueError(
                    "You are attempting to use Flash Attention 2.0 with a model not initialized on GPU and with no GPU"
                    "available."
                    "This is not supported yet. Please make sure to have access to a GPU and either initialise the"
                    "model on a GPU by passing a device_map"
            ...
            raise ValueError(
                "You are attempting to use Flash Attention 2.0 with a model dispatched on CPU or disk. This is not"
                "supported. Please make sure to"
    ...
    def _check_and_enable_sdpa(cls, config, hard_check_only: bool = False) -> PretrainedConfig:
        ...
        If all checks pass and `hard_check_only` is False, the method
        will set the config attribute `_attn_implementation` to
        "flash_attention_2" so that the model can initialize the
        correct attention module.
        """
        if hard_check_only:
            if not cls._supports_sdpa:
                raise ValueError(
                    ...
                    f"{cls.__name__} does not support an attention implementation through"
                    f"torch.nn.functional.scaled_dot_product_attention yet."
                    " Please request the support for this architecture:"
                    "https://github.com/huggingface/transformers/issues/28005. If you believe"
                    " this error is a bug, please open an issue in Transformers GitHub repository and load your model"
                    'with the argument `attn_implementation="eager"` meanwhile. Example: `model ='
                    'AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="eager")`'
                    " this error is a bug, please open an issue in Transformers GitHub repository and load your model"
                    'with the argument `attn_implementation="eager"` meanwhile. Example: `model ='
                    'AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="eager")`'
                )
            ...
            if module.__class__.__name__ not in _no_split_modules:
                if isinstance(module, PreTrainedModel):
                    if module._no_split_modules is None:
                        raise ValueError(
                            f"{module.__class__.__name__} does not support"
                            f" `device_map='{device_map}'`. To implement"
                            f" support, the model"
    ...
    def resize_token_embeddings(
        ...
        Arguments:
            ...
            mean_resizing (`bool`):
                Whether to initialize the added embeddings from a
                multivariate normal distribution that has old embeddings'
                mean and covariance or to initialize them with a normal
                distribution that has a mean of zero and std equals
                `config.initializer_range`.

                Setting `mean_resizing` to `True` is useful when
                increasing the size of the embeddings of causal
                language models,
                where the generated tokens' probabilities won't be
                affected by the added embeddings because initializing
                the new embeddings with the old embeddings' mean will
                reduce the kl-divergence between the next token
                probability before and after adding the new embeddings.
    ...
    def _get_resized_embeddings(
        ...
        Args:
            ...
            mean_resizing (`bool`):
                ...
                increasing the size of the embeddings of causal
                language models,
                where the generated tokens' probabilities will not be
                affected by the added embeddings because initializing
                the new embeddings with the old embeddings' mean will
                reduce the kl-divergence between the next token
                probability before and after adding the new embeddings.
        ...
        if pad_to_multiple_of is not None:
            if not isinstance(pad_to_multiple_of, int):
                raise ValueError(
                    f"Asking to pad the embedding matrix to a"
                    f" multiple of `{pad_to_multiple_of}`, which"
                    f" is not an integer. Please make sure to"
                    f" pass an integer"
                )
            ...
            logger.info(
                "You are resizing the embedding layer without providing a `pad_to_multiple_of` parameter. This means"
                "that the new embedding"
                f" dimension will be {new_num_tokens}. This might"
                f" induce some performance reduction as"
                f" *Tensor Cores* will not be available."
        ...
        if new_num_tokens > old_num_tokens and not mean_resizing:
            # initialize new embeddings (in particular added tokens) with a mean of 0 and std equals
            # `config.initializer_range`.
            ...
            logger.warning_once(
                ...
                "embeddings' mean and covariance."
                "As described in this article: https://nlp.stanford.edu/~johnhew/vocab-expansion.html. "
        ...
        # The input and output embedding types remain consistent. (c.f.
        # https://github.com/huggingface/transformers/pull/31979)
        if is_deepspeed_zero3_enabled() and not is_quantized:
            import deepspeed
    ...
    def _get_resized_lm_head(
        ...
        if new_num_tokens > old_num_tokens and not mean_resizing:
            ...
            # as described in this article: https://nlp.stanford.edu/~johnhew/vocab-expansion.html
            logger.warning_once(
                "The new lm_head weights will be initialized from a multivariate normal distribution that has old"
                "embeddings' mean and covariance."
                "As described in this article: https://nlp.stanford.edu/~johnhew/vocab-expansion.html. "
                "To disable this, use `mean_resizing=False`"
            )
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None):
        ...
        if not _is_using_old_format:
            ...
            logger.warning(
                ...
                "ignore `gradient_checkpointing_kwargs` in case you passed it)."
                "Please update to the new format on your modeling file. To use the new format, you need to completely"
                "remove the definition of the method `_set_gradient_checkpointing` in your model."
            )

        if getattr(self, "_hf_peft_config_loaded", False):
            # When using PEFT + gradient checkpointing + Trainer we need to make sure the input has requires_grad=True
            # we do it also on PEFT: https://github.com/huggingface/peft/blob/85013987aa82aa1af3da1236b6902556ce3e483e/s
            # rc/peft/peft_model.py#L334
            # When training with PEFT, only LoRA layers will have requires grad set to True, but the output of frozen
            # layers need to propagate
    ...
    def _set_gradient_checkpointing(self, enable: bool = True, gradient_checkpointing_func: Callable = checkpoint):
        ...
        if not is_gradient_checkpointing_set:
            raise ValueError(
                ...
                f" gradient checkpointing. Make sure all the"
                f" architecture support it by setting a boolean attribute"
        ...
        if self.supports_gradient_checkpointing:
            ...
            if not _is_using_old_format:
                ...
                logger.warning(
                    "You are using an old version of the checkpointing format that is deprecated (We will also silently"
                    "ignore `gradient_checkpointing_kwargs` in case you passed it)."
                    "Please update to the new format on your modeling file. To use the new format, you need to"
                    "completely remove the definition of the method `_set_gradient_checkpointing` in your model."
                )
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if hf_quantizer is not None and not _hf_peft_config_loaded and not quantization_serializable:
            raise ValueError(
                f"The model is quantized with"
                f" {hf_quantizer.quantization_config.quant_method}"
                f" and is not serializable - check out the warnings from"
        ...
        if is_main_process:
            ...
            if _hf_peft_config_loaded:
                logger.info(
                    "Detected adapters on the model, saving the model in the PEFT format, only adapter weights will be"
                    "saved."
                )
                ...
                if len(active_adapter) > 1:
                    raise ValueError(
                        "Multiple active adapters detected, saving multiple active adapters is not supported yet. You"
                        "can save adapters separately one by one"
        ...
        if state_dict is None:
            ...
            if (
                ...
                warnings.warn(
                    "Attempting to save a model with offloaded modules. Ensure that unallocated cpu memory exceeds the"
        ...
        if safe_serialization:
            ...
            if len(error_names) > 0:
                raise RuntimeError(
                    f"The weights trying to be saved contained shared tensors {error_names} that are mismatching the"
                    f"transformers base configuration. Try saving using `safe_serialization=False` or remove this"
                    f"tensor sharing.",
                    f"The weights trying to be saved contained shared tensors {error_names} that are mismatching the"
                    f"transformers base configuration. Try saving using `safe_serialization=False` or remove this"
                    f"tensor sharing.",
        ...
        for shard_file, tensors in filename_to_tensors:
            ...
            if module_map:
                if accelerate_version < version.parse("0.31"):
                    raise ImportError(
                        f"You need accelerate version to be greater or equal than 0.31 to save models with offloaded"
                        f"parameters. Detected version {accelerate_version}."
                        f"You need accelerate version to be greater or equal than 0.31 to save models with offloaded"
                        f"parameters. Detected version {accelerate_version}."
        ...
        if index is None:
            ...
            logger.info(
                ...
                f"split in {len(state_dict_split.filename_to_tensors)}"
                f" checkpoint shards. You can find where each"
                f" parameters has been saved in the"
    ...
    def cuda(self, *args, **kwargs):
        ...
        if getattr(self, "quantization_method", None) == QuantizationMethod.BITS_AND_BYTES:
            if getattr(self, "is_loaded_in_8bit", False):
                ...
                raise ValueError(
                    "Calling `cuda()` is not supported for `4-bit` quantized models with the installed version of"
                    "bitsandbytes."
                    f"The current device is `{self.device}`."
                    f" If you intended to move the model,"
                    f" please install bitsandbytes >= 0.43.2."
                )
        else:
    ...
    def to(self, *args, **kwargs):
        ...
        if getattr(self, "quantization_method", None) == QuantizationMethod.BITS_AND_BYTES:
            if dtype_present_in_args:
                raise ValueError(
                    "You cannot cast a bitsandbytes model in a new `dtype`. Make sure to load the model using"
                    "`from_pretrained` using the"
            ...
            if getattr(self, "is_loaded_in_8bit", False):
                ...
                raise ValueError(
                    "Calling `to()` is not supported for `4-bit` quantized models with the installed version of"
                    "bitsandbytes."
                    f"The current device is `{self.device}`."
                    f" If you intended to move the model,"
                    f" please install bitsandbytes >= 0.43.2."
            ...
            if dtype_present_in_args:
                raise ValueError(
                    "You cannot cast a GPTQ model in a new `dtype`. Make sure to load the model using `from_pretrained`"
                    "using the desired"
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        If model weights are the same precision as the base model
        (and is a supported model), weights will be lazily loaded
        ...
        Parameters:
            ...
            output_loading_info(`bool`, *optional*, defaults to `False`):
                Whether to not to also return a dictionary containing missing keys, unexpected keys and error messages.
            ...
            attn_implementation (`str`, *optional*):
                ...
                implementation of the attention), `"sdpa"` (using
                `F.scaled_dot_product_attention`), or
                `"flash_attention_2"` (using Dao-AILab/flash-attention).
                By default, if available, SDPA will be used for
                torch>=2.1.1. The default is otherwise the manual
            ...
            torch_dtype (`str` or `torch.dtype`, *optional*):
                ...
                3. A string that is a valid `torch.dtype`. E.g. "float32"
                loads the model in `torch.float32`, "float16" loads in
                `torch.float16` etc.
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        if commit_hash is None:
            if not isinstance(config, PretrainedConfig):
                # We make a call to the config file first (which may be absent) to get the commit hash as soon as
                # possible
        ...
        if isinstance(device_map, torch.device):
            ...
            if device_map < 0:
                raise ValueError(
                    "You can't pass device_map as a negative int. If you want to put the model on the cpu, pass"
                    "device_map = 'cpu'"
                )
            else:
                device_map = {"": device_map}
        ...
        if low_cpu_mem_usage:
            if is_deepspeed_zero3_enabled():
                ...
                raise ImportError(
                    f"Using `low_cpu_mem_usage=True` or a `device_map`"
                    f" requires Accelerate: `pip install"
        ...
        if load_in_4bit or load_in_8bit:
            ...
            logger.warning(
                "The `load_in_4bit` and `load_in_8bit` arguments are deprecated and will be removed in the future"
                "versions."
        ...
        if gguf_file is not None and hf_quantizer is not None:
            raise ValueError(
                "You cannot combine Quantization and loading a model from a GGUF file, try again by making sure you did"
                "not passed a `quantization_config` or that you did not load a quantized model from the Hub."
            )
                        ...
                        if resolved_archive_file is not None:
                            ...
                            if resolved_archive_file is None:
                                raise OSError(
                                    ...
                                    f" {_add_variant(SAFE_WEIGHTS_NAME, variant)}"
                                    f" or {_add_variant(SAFE_WEIGHTS_INDEX_NAME, variant)} "
                    ...
                    if not local_files_only and not is_offline_mode():
                        if resolved_archive_file is not None:
                            ...
                            if has_file(pretrained_model_name_or_path, TF2_WEIGHTS_NAME, **has_file_kwargs):
                                raise OSError(
                                    f"{pretrained_model_name_or_path} does not appear to have a file named"
                                    f" {_add_variant(WEIGHTS_NAME, variant)} but there is a file for TensorFlow"
                                    f"weights."
                                    f" {_add_variant(WEIGHTS_NAME, variant)}"
                                    f" but there is a file for TensorFlow weights."
        ...
        if (
            ...
            if metadata.get("format") == "pt":
                ...
                raise ValueError(
                    f"Incompatible safetensors file. File metadata is"
                    f" not ['pt', 'tf', 'flax', 'mlx'] but"
            ...
            if torch_dtype is not None:
                if isinstance(torch_dtype, str):
                    if torch_dtype == "auto":
                        ...
                        raise ValueError(
                            f"`torch_dtype` can be one of: `torch.dtype`,"
                            f' `"auto"` or a string of a valid'
                            f" `torch.dtype`, but received {torch_dtype}"
                        )
                dtype_orig = cls._set_default_torch_dtype(torch_dtype)
    ...
    def _load_pretrained_model(
                ...
                for checkpoint_key in loaded_keys:
                    ...
                    if (
                        ...
                        if (
                            ...
                            # This skips size mismatches for 4-bit weights. Two 4-bit values share an 8-bit container,
                            # causing size differences.
                            # Without matching with module type or parameter
                            # type it seems like a practical way to detect
                            # valid 4bit weights.
                            pass
                        else:
        ...
        if state_dict is not None:
            ...
            for shard_file in resolved_archive_file:
                # Skip the load for shards that only contain disk-offloaded weights when using safetensors for the
                # offload.
    ...
    def _is_quantized_training_enabled(self):
        warnings.warn(
            ...
            " transformers 4.39.0. Please use"
            " `model.hf_quantizer.is_trainable` instead",
            FutureWarning,
        )
    ...
    if is_accelerate_available():
        ...
        if recursive:
            if not is_accelerate_available("0.29.0"):
                raise RuntimeError("Setting `recursive=True` to `unwrap_model` requires `accelerate` v0.29.0.")
            else:
...
def expand_device_map(device_map, param_names, start_prefix):
    """Expand a device map to return parameter name to device mapping."""
    new_device_map = {}
...
def get_disk_only_shard_files(device_map, sharded_metadata, start_prefix):
    """Returns the list of shard files containing only
    weights offloaded to disk."""
    weight_map = {

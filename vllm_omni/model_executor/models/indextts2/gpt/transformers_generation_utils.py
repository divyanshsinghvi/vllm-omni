...
def _crop_past_key_values(model, past_key_values, max_length):
    """Crop past key values to a given maximum length.
    Removed from transformers >= 4.57; kept as a local stub."""
...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        logits (`tuple(torch.FloatTensor)` *optional*, returned when `output_logits=True`):
            Unprocessed prediction scores of the language modeling head
...
class GenerateEncoderDecoderOutput(ModelOutput):
    ...
    Args:
        ...
        cross_attentions (`tuple(tuple(torch.FloatTensor))`, *optional*, returned when `output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `torch.FloatTensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
        decoder_hidden_states (`tuple(tuple(torch.FloatTensor))`, *optional*,
...
class GenerateBeamDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        sequences (`torch.LongTensor` of shape `(batch_size*num_return_sequences, sequence_length)`):
            The generated sequences. The second dimension (sequence_length) is either equal to `max_length` or shorter
            if all batches finished early due to the `eos_token_id`.
        sequences_scores (`torch.FloatTensor` of shape
            `(batch_size*num_return_sequences)`, *optional*,
...
class GenerationMixin:
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if decoder_start_token_id.ndim == 1:
            if decoder_start_token_id.shape[0] != batch_size:
                raise ValueError(
                    f"`decoder_start_token_id` expected to have length"
                    f" {batch_size} but got"
                    f" {decoder_start_token_id.shape[0]}"
                )
    ...
    def _get_logits_processor(
        ...
        if generation_config.do_sample:
            ...
            # the following idea is largely copied from this PR:
            # https://github.com/huggingface/transformers/pull/5420/files
            ...
            if generation_config.min_p is not None:
                # Applied after temperature scaling (see
                # https://github.com/ggerganov/llama.cpp/pull/3841#issuecomment-2073826084)
                processors.append(
    ...
    def compute_transition_scores(
        ...
        Parameters:
            ...
            scores (`tuple(torch.FloatTensor)`):
                ...
                of log probabilities of tokens conditioned on log softmax of previously generated tokens in this beam.
                Tuple of `torch.FloatTensor` with up to `max_new_tokens`
    ...
    def _validate_assistant(self, assistant_model, tokenizer, assistant_tokenizer):
        ...
        if self.config.is_encoder_decoder and not assistant_model.config.is_encoder_decoder:
            ...
            if not are_equal:
                raise ValueError(
                    ...
                    "Ensure you load the assistant with the correct encoder-decoder class, e.g."
        ...
        if self.config.get_text_config().vocab_size == assistant_model.config.get_text_config().vocab_size:
            ...
            if tokenizer is None or assistant_tokenizer is None:
                raise ValueError(
                    f"The main and assistant moedels have different tokenizers. Please provide `tokenizer` and"
                    f"`assistant_tokenizer` to `generate()` {doc_reference}."
                    f"The main and assistant moedels have different tokenizers. Please provide `tokenizer` and"
                    f"`assistant_tokenizer` to `generate()` {doc_reference}."
                )
    ...
    def _prepare_generation_config(
        ...
        """
        # TODO joao: when we can detect `fullgraph=True` in `torch.compile`
        # (https://github.com/pytorch/pytorch/pull/120400)
    ...
    def _has_unfinished_sequences(
        ...
        # TODO (joao): remove this when torch's support for control flow is not experimental
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if model_kwargs.get("past_key_values") is None or (
                ...
                # Clone is needed to avoid keeping a hanging ref to outputs.logits which may be very large for this
                # first iteration
...
def _relative_top_filter(
    ...
    """
    Reference:
    https://github.com/XiangLi1999/ContrastiveDecoding/blob/170e914/
    transformers/src/transformers/generation_logits_process.py#L235

    Apply filtering to only keep tokens with a probability above a
    certain threshold. The threshold is defined as `relative_top` *
    max probability in the distribution.
    """
